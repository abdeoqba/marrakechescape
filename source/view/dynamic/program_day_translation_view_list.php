<?php
  ...
  $p->getAllProgramDayTranslation($page_number);
  

  $program_day_ = new ProgramDay();
  $program_day_->getAllProgramDay();
  $nbr_program_days = count($program_day_->program_days);
...
?>
      ...
      <div class="container-fluid py-5">
        <div class="row">
          <div class="col-lg-6 col-md-6">
            <h2>List of Program Day Translation <span class="badge badge-primary badge-pill" style="font-size:11pt;"><?=count($p->program_day_translations); ?></span></h2>
          </div>
          <div class="col-lg-6 col-md-6 text-right pb-3">
            ...
            <!-- truncate -->
            <button type="button" class="btn btn-danger d-none" onclick="program_day_translation_truncate_set();" <?php 
            if(!$_SESSION[variables::$prefix."allow_truncate"]) echo "disabled";
            ?>>
              <i class="fa fa-trash-alt"></i> Truncate Program Day Translation
            </button>
            
            <!-- trash link -->
            <?php 
            if($_SESSION[variables::$prefix."allow_trash"]){
              $p->getNbrDeletedProgramDayTranslation(); 
            ?>
            <a href="program_day_translations-trash" class="btn btn-danger btn-fixed btn-<?=$nbr_btn; ?>">
            ...
            <button type="button" class="btn btn-primary btn-fixed btn-<?=$nbr_btn; ?>" data-toggle="modal" data-target="#ProgramDayTranslationAddModal">
              <i class="fa fa-plus fa-lg"></i>
              <span class="hover-show">
                Add Program Day Translation
              </span>
            </button>
            <?php   $nbr_btn++; ?>
            <?php } ?>
    ...
    <?php if($_SESSION[variables::$prefix."allow_edit"]){ ?>
      ...
      <?php include(variables::$root."/source/view/modal/program_day_translation_section_modal_form_add.php"); ?>
      <?php include(variables::$root."/source/view/modal/program_day_translation_section_modal_form_update.php"); ?>
      <?php include(variables::$root."/source/view/modal/program_day_translation_section_modal_confirm_move_to_trash.php"); ?>
      
      <?php if(($_SESSION[variables::$prefix."allow_truncate"])){ ?>
        <?php include(variables::$root."/source/view/modal/program_day_translation_section_modal_confirm_truncate.php"); ?>
