<?php
  ...
  $p->getAllPostTranslation($page_number);
  

  $post_ = new Post();
  $post_->getAllPost();
  $nbr_posts = count($post_->posts);
...
?>
      ...
      <div class="container-fluid py-5">
        <div class="row">
          <div class="col-lg-6 col-md-6">
            <h2>List of Post Translation <span class="badge badge-primary badge-pill" style="font-size:11pt;"><?=count($p->post_translations); ?></span></h2>
          </div>
          <div class="col-lg-6 col-md-6 text-right pb-3">
            ...
            <!-- truncate -->
            <button type="button" class="btn btn-danger d-none" onclick="post_translation_truncate_set();" <?php 
            if(!$_SESSION[variables::$prefix."allow_truncate"]) echo "disabled";
            ?>>
              <i class="fa fa-trash-alt"></i> Truncate Post Translation
            </button>
            
            <!-- trash link -->
            <?php 
            if($_SESSION[variables::$prefix."allow_trash"]){
              $p->getNbrDeletedPostTranslation(); 
            ?>
            <a href="post_translations-trash" class="btn btn-danger btn-fixed btn-<?=$nbr_btn; ?>">
            ...
            <button type="button" class="btn btn-primary btn-fixed btn-<?=$nbr_btn; ?>" data-toggle="modal" data-target="#PostTranslationAddModal">
    ...
    <?php if($_SESSION[variables::$prefix."allow_edit"]){ ?>
      ...
      <?php include(variables::$root."/source/view/modal/post_translation_section_modal_form_add.php"); ?>
      <?php include(variables::$root."/source/view/modal/post_translation_section_modal_form_update.php"); ?>
      <?php include(variables::$root."/source/view/modal/post_translation_section_modal_confirm_move_to_trash.php"); ?>
      
      <?php if(($_SESSION[variables::$prefix."allow_truncate"])){ ?>
        <?php include(variables::$root."/source/view/modal/post_translation_section_modal_confirm_truncate.php"); ?>
